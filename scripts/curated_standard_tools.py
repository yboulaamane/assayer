...
T = [
    ...
    ("AIMNet2", "qm", "Neural network potential covering neutral, charged and open-shell organic molecules across most of the periodic table — QM-quality energies at force-field cost.", "https://github.com/isayevlab/aimnetcentral", "isayevlab/aimnetcentral", "open-source", ["mlip", "semiempirical-replacement"]),
    ...
    ("Chemformer", "synthesis", "Sequence-to-sequence transformer for reaction prediction and retrosynthesis, pretrained on SMILES and fine-tuned per task. Now maintained inside AiZynthModels; the standalone repository is archived.", "https://github.com/MolecularAI/aizynthmodels", "MolecularAI/aizynthmodels", "open-source", ["retrosynthesis", "reaction-prediction"]),
    ...
    ("SwissTargetPrediction", "target-id", "Predicts a small molecule's likely protein targets by combining 2D and 3D similarity to compounds with measured activity. The web standard for a first pass, with the caveat that it can only propose targets that already have ligands.", "https://www.swisstargetprediction.ch", None, "free-web", ["off-target", "polypharmacology", "ligand-based"]),
    ("PPB2", "target-id", "Polypharmacology Browser 2: nearest-neighbour search over ChEMBL by ECfp4, Xfp shape-pharmacophore or MQN fingerprints, optionally combined with a naive Bayes model fitted on the fly. Useful because you can compare what different descriptors nominate.", "https://ppb2.gdb.tools", None, "free-web", ["off-target", "polypharmacology", "fingerprints"]),
    ("TargetNet", "target-id", "Runs a per-target QSAR model across 623 human proteins and returns a predicted activity profile rather than a ranked shortlist. Each target carries its own model, so applicability varies target by target.", "http://targetnet.scbdd.com", None, "free-web", ["off-target", "qsar", "profiling"]),
    ("ChEMBL multitask target prediction", "target-id", "ChEMBL's own multitask neural network for target prediction, shipped as a model you run locally instead of a web form. Trained on ChEMBL activities, with runnable examples in Python, C++, Julia and JavaScript.", "https://github.com/chembl/chembl_multitask_model", "chembl/chembl_multitask_model", "open-source", ["off-target", "neural-network", "local"]),
    ...
    ("AiZynthModels", "synthesis", "AstraZeneca's repository for training, evaluating and using synthesis prediction models, and the replacement for AiZynthTrain. Train AiZynthFinder's models on your own reaction data so in-house chemistry and stock are reflected in the routes.", "https://github.com/MolecularAI/aizynthmodels", "MolecularAI/aizynthmodels", "open-source", ["retrosynthesis", "training", "in-house"]),
    ...
    ("DockStream", "docking", "AstraZeneca's docking wrapper exposing several backends to generative loops such as REINVENT. Archived and unmaintained since 2023: usable as a reference, but plan to pin its dependencies.", "https://github.com/MolecularAI/DockStream", "MolecularAI/DockStream", "open-source", ["archived", "docking", "generative-loop"]),
    ...
    ("DEL-iver", "qsar-ml", "Processes DNA-encoded library selections: building-block and disynthon enrichment, binding models on building-block or full-molecule inputs, and analog picking from make-on-demand space. Early research code from a 2026 preprint. The default split is random, which leaks building blocks between train and test, so re-split by building block before trusting the AUROC.", "https://github.com/SztainLab/DEL-iver", "SztainLab/DEL-iver", "open-source", ["dna-encoded-library", "hit-finding", "enrichment"]),
    ...
    ("ESM", "biologics", "Evolutionary-scale protein language models (ESM-2, ESMFold), used for embeddings, variant effect scores and as the front end of structure prediction without an MSA. This repository is archived; the newer models in the line (ESM C, ESMFold2, ESM3) are developed at Biohub/esm under their own licence.", "https://github.com/facebookresearch/esm", "facebookresearch/esm", "open-source", ["archived", "protein-language-model", "variant-effect"]),
